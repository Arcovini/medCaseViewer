...
1. `POST /upload` with a `FormData` containing each STL under the repeated field name `files` (not `files[]`). Response is immediate and contains `{uid, viewer_url, ...}`.
   - The idle state is a **two-step wizard** inside one page: step 1 picks the files, step 2 configures divisions. It is UI navigation only — still a single `POST /upload` at the end with files + `boolean_ops` together (the backend is stateless, so uploading at step 1 would need server-side state). Step 2 and the stepper only exist when there is something to divide (2+ STLs); a single file or an OBJ bundle stays a one-screen flow with `Processar` on step 1. `#btn-continue` / `#btn-back` navigate; state survives going back.
