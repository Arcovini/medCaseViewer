...
function createToggleButton(btnType, instance, name) {
    ...
    // COR ALTERADA NAS IMAGENS
    btn.innerHTML = `<img id="imggEye" src="eye_icon_colored.svg" width="24" alt="Hide" />`;
    return btn;
}
    ...
    for (let i = 0; i < toggleButtons.length; i++) {
        toggleButtons[i].addEventListener("click", function () {
            if (this.dataset.isHidden === "false") {
                ...
                // COR ALTERADA NAS IMAGENS
                this.innerHTML = `<img src="eye_off_icon_colored.svg" width="24" alt="Show" />`;
            } else {
                api.show(this.value);
                this.dataset.isHidden = "false";
                // COR ALTERADA NAS IMAGENS
                this.innerHTML = `<img src="eye_icon_colored.svg" width="24" alt="Hide" />`;
                ...
                if (parentEl) {
                    ...
                    for (let j = 0; j < childToggles.length; j++) {
                        ...
                        // COR ALTERADA NAS IMAGENS
                        childToggles[j].innerHTML = `<img src="eye_icon_colored.svg" width="24" alt="Hide" />`;
                    }
                }
            }
        });
...
function createOpacitySlider(opacity, materialIndex) {
    ...
    slider.max = "1";
    slider.step = "0.01";
    slider.style.color = "rgba(224, 212, 212, 1)";
    slider.style.backgroundColor = "rgba(224, 212, 212, 1)";
